...
//var csv = './data/sample2.csv';
//var geo = './data/alternative_country_topo.json';
...
module.exports = React.createClass({
  ...
  componentWillMount: function() {
    ...
    function _render(rows) {
      ...
      self.setState({indicatorList: indicators});
  ...
  componentWillUpdate: function(nextProps, nextState) {
    ...
    this.updateChoropleth(nextState.csv_data, nextState.geo_data, nextState.indicator, nextState.map);
     ...
     // Check if all countries are in dataset
    // Object.keys(nextState.csv_data).map(function(row) {
    //   var country = nextState.csv_data[row]['Country'];
    //   var found = false;
    //   for (var layer_i in nextState.map.choropleth._layers) {
    //     var name = nextState.map.choropleth._layers[layer_i].feature.properties.name;
    //     if (name == country){
    //       found = true;
    //       break;
    //     }
    //   }
    //   if (!found){
    //     console.log('Can not find country', country);
    //   }
  ...
  handleClick: function(){
    ...
    var index = Math.round(Math.random()*(this.state.indicatorList.length - 1))
    this.setState({indicator: this.state.indicatorList[index]});
...
=======
      <div>
        <button style={mstyle} onClick={this.handleClick}>{this.state.indicator}</button>
>>>>>>> Add tooltips to map. Use topojson. Better data handling.
        <div id='map'></div>
      </section>
    );
  }
